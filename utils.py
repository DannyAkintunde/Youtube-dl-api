...
def get_captions(yt,lang):
    try:
      ...
      if transcript:
        ...
        srt_caption = SRTFormatter().format_transcript(caption)
        with open(os.path.join(TEMP_DIR, f"{yt.title}.srt"), 'w', encoding='utf-8') as file:
        ...
        return {"lang": lang, "lang_code": transcript.language_code,"caption": text_caption,"caption_json": json_caption, "file":caption_file, "path":os.path.join(TEMP_DIR, f"{yt.title}.srt")}, None
      else:
